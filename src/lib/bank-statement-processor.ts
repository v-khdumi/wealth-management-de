...
function generateFallbackExtractedData(
  ...
  const monthMatch = fileName.toLowerCase().match(/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*\b/)
  if (monthMatch) {
    const idx = monthNames.indexOf(monthMatch[1].toLowerCase().slice(0, 3))
